...
<main id="primary" class="contact-page">
	...
	<section class="ct-main" id="contact-form"><div class="container"><div class="ct-grid">
		<aside class="ct-info">
			...
			<?php $socials = blue_social_links(); if ( $socials ) : ?><div class="ct-follow"><span class="ct-follow-label"><?php echo esc_html( blue_text( 'Follow Blue', 'تابع بلو' ) ); ?></span><div class="social-row on-light"><?php foreach ( $socials as $social ) : $social_icon = blue_social_icon_key( $social ); $social_icon_markup = blue_icon_image_html( $social['icon_image'] ?? '', 'social-icon-image' ) ?: blue_social_icon_svg( $social_icon ); ?><a class="social-ic" href="<?php echo esc_url( $social['url'] ?? '' ); ?>" target="_blank" rel="noopener" aria-label="<?php echo esc_attr( $social['label'] ?? '' ); ?>"><?php echo $social_icon_markup; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped image or fixed theme SVG. ?></a><?php endforeach; ?></div></div><?php endif; ?>
		</aside>
