...
/**
 * Do not let Polylang filter shared WooCommerce records out of English URLs.
 *
 * The /en/ path and blue_lang keep the storefront context in English. Adding
 * lang=en to a product taxonomy query, however, asks Polylang for products
 * assigned to its language taxonomy. Products and their taxonomies are shared
 * by this theme, so that filter produces an empty archive. This also repairs
 * requests generated by rewrite rules cached before this correction.
 */
add_filter(
	'request',
	function ( array $query_vars ): array {
		if ( 'en' !== ( $query_vars['blue_lang'] ?? '' ) ) {
			return $query_vars;
		}

		$taxonomy = (string) ( $query_vars['taxonomy'] ?? '' );
		$shared   = ! empty( $query_vars['product_cat'] )
			|| ! empty( $query_vars['product_tag'] )
			|| ! empty( $query_vars['product_shipping_class'] )
			|| ! empty( $query_vars['product'] )
