<?php
namespace BlueMattress\ProductQR;
// Vendored QR encoder; constants prefixed to avoid plugin collisions.
...
define("BLUE_PRODUCT_QR_PAD0", 0xEC);
define("BLUE_PRODUCT_QR_PAD1", 0x11);
...
class QRCode {
    ...
    function __construct() {
        $this->typeNumber = 1;
        $this->errorCorrectLevel = BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_H;
    ...
    function addData($data, $mode = 0) {
        ...
        case BLUE_PRODUCT_QR_MODE_NUMBER :
            $this->addDataImpl(new QRNumber($data) );
            break;

        case BLUE_PRODUCT_QR_MODE_ALPHA_NUM :
            $this->addDataImpl(new QRAlphaNum($data) );
            break;

        case BLUE_PRODUCT_QR_MODE_8BIT_BYTE :
            $this->addDataImpl(new QR8BitByte($data) );
            break;

        case BLUE_PRODUCT_QR_MODE_KANJI :
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        while (true) {
            ...
            $buffer->put(BLUE_PRODUCT_QR_PAD0, 8);

            if ($buffer->getLengthInBits() >= $totalDataCount * 8) {
                break;
            }
            $buffer->put(BLUE_PRODUCT_QR_PAD1, 8);
        }
...
define("BLUE_PRODUCT_QR_G15", (1 << 10) | (1 << 8) | (1 << 5)
    | (1 << 4) | (1 << 2) | (1 << 1) | (1 << 0) );

define("BLUE_PRODUCT_QR_G18", (1 << 12) | (1 << 11) | (1 << 10)
    | (1 << 9) | (1 << 8) | (1 << 5) | (1 << 2) | (1 << 0) );

define("BLUE_PRODUCT_QR_G15_MASK", (1 << 14) | (1 << 12) | (1 << 10)
    | (1 << 4) | (1 << 1) );

class QRUtil {

    static $BLUE_PRODUCT_QR_MAX_LENGTH = array(
    ...
    static $BLUE_PRODUCT_QR_PATTERN_POSITION_TABLE = array(
        array(),
        array(6, 18),
        array(6, 22),
        array(6, 26),
    ...
    static function getPatternPosition($typeNumber) {
        return self::$BLUE_PRODUCT_QR_PATTERN_POSITION_TABLE[$typeNumber - 1];
    }
    ...
    static function getMaxLength($typeNumber, $mode, $errorCorrectLevel) {
        ...
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_L : $e = 0; break;
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_M : $e = 1; break;
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_Q : $e = 2; break;
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_H : $e = 3; break;
        default :
            trigger_error("e:$errorCorrectLevel", E_USER_ERROR);
        }

        switch($mode) {
        case BLUE_PRODUCT_QR_MODE_NUMBER    : $m = 0; break;
        case BLUE_PRODUCT_QR_MODE_ALPHA_NUM : $m = 1; break;
        case BLUE_PRODUCT_QR_MODE_8BIT_BYTE : $m = 2; break;
        case BLUE_PRODUCT_QR_MODE_KANJI     : $m = 3; break;
        default :
            trigger_error("m:$mode", E_USER_ERROR);
        }

        return self::$BLUE_PRODUCT_QR_MAX_LENGTH[$t][$e][$m];
    }
    ...
    static function getMask($maskPattern, $i, $j) {
        ...
        case BLUE_PRODUCT_QR_MASK_PATTERN000 : return ($i + $j) % 2 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN001 : return $i % 2 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN010 : return $j % 3 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN011 : return ($i + $j) % 3 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN100 : return (floor($i / 2) + floor($j / 3) ) % 2 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN101 : return ($i * $j) % 2 + ($i * $j) % 3 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN110 : return ( ($i * $j) % 2 + ($i * $j) % 3) % 2 == 0;
        case BLUE_PRODUCT_QR_MASK_PATTERN111 : return ( ($i * $j) % 3 + ($i + $j) % 2) % 2 == 0;
    ...
    static function getMode($s) {
        if (QRUtil::isAlphaNum($s) ) {
            if (QRUtil::isNumber($s) ) {
                return BLUE_PRODUCT_QR_MODE_NUMBER;
            }
            return BLUE_PRODUCT_QR_MODE_ALPHA_NUM;
        } else if (QRUtil::isKanji($s) ) {
            return BLUE_PRODUCT_QR_MODE_KANJI;
        } else {
            return BLUE_PRODUCT_QR_MODE_8BIT_BYTE;
        }
    }
    ...
    static function getBCHTypeInfo($data) {
        $d = $data << 10;
        while (QRUtil::getBCHDigit($d) - QRUtil::getBCHDigit(BLUE_PRODUCT_QR_G15) >= 0) {
            $d ^= (BLUE_PRODUCT_QR_G15 << (QRUtil::getBCHDigit($d) - QRUtil::getBCHDigit(BLUE_PRODUCT_QR_G15) ) );
        }
        return ( ($data << 10) | $d) ^ BLUE_PRODUCT_QR_G15_MASK;
    }

    static function getBCHTypeNumber($data) {
        $d = $data << 12;
        while (QRUtil::getBCHDigit($d) - QRUtil::getBCHDigit(BLUE_PRODUCT_QR_G18) >= 0) {
            $d ^= (BLUE_PRODUCT_QR_G18 << (QRUtil::getBCHDigit($d) - QRUtil::getBCHDigit(BLUE_PRODUCT_QR_G18) ) );
        }
        return ($data << 12) | $d;
    }
...
class QRRSBlock {
    ...
    static $BLUE_PRODUCT_QR_RS_BLOCK_TABLE = array(
    ...
    static function getRsBlockTable($typeNumber, $errorCorrectLevel) {
        ...
        switch($errorCorrectLevel) {
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_L :
            return self::$BLUE_PRODUCT_QR_RS_BLOCK_TABLE[($typeNumber - 1) * 4 + 0];
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_M :
            return self::$BLUE_PRODUCT_QR_RS_BLOCK_TABLE[($typeNumber - 1) * 4 + 1];
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_Q :
            return self::$BLUE_PRODUCT_QR_RS_BLOCK_TABLE[($typeNumber - 1) * 4 + 2];
        case BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_H :
            return self::$BLUE_PRODUCT_QR_RS_BLOCK_TABLE[($typeNumber - 1) * 4 + 3];
        default :
...
class QRNumber extends QRData {
    ...
    function __construct($data) {
        parent::__construct(BLUE_PRODUCT_QR_MODE_NUMBER, $data);
    }
...
class QRKanji extends QRData {
    ...
    function __construct($data) {
        parent::__construct(BLUE_PRODUCT_QR_MODE_KANJI, $data);
    }
...
class QRAlphaNum extends QRData {
    ...
    function __construct($data) {
        parent::__construct(BLUE_PRODUCT_QR_MODE_ALPHA_NUM, $data);
    }
...
class QR8BitByte extends QRData {
    ...
    function __construct($data) {
        parent::__construct(BLUE_PRODUCT_QR_MODE_8BIT_BYTE, $data);
    }
...
abstract class QRData {
    ...
    function getLengthInBits($type) {
        ...
        if (1 <= $type && $type < 10) {
            ...
            switch($this->mode) {
            case BLUE_PRODUCT_QR_MODE_NUMBER     : return 10;
            case BLUE_PRODUCT_QR_MODE_ALPHA_NUM     : return 9;
            case BLUE_PRODUCT_QR_MODE_8BIT_BYTE    : return 8;
            case BLUE_PRODUCT_QR_MODE_KANJI      : return 8;
            default :
            ...
            switch($this->mode) {
            case BLUE_PRODUCT_QR_MODE_NUMBER     : return 12;
            case BLUE_PRODUCT_QR_MODE_ALPHA_NUM     : return 11;
            case BLUE_PRODUCT_QR_MODE_8BIT_BYTE    : return 16;
            case BLUE_PRODUCT_QR_MODE_KANJI      : return 10;
            default :
                trigger_error("mode:$this->mode", E_USER_ERROR);
            }
            ...
            switch($this->mode) {
            case BLUE_PRODUCT_QR_MODE_NUMBER     : return 14;
            case BLUE_PRODUCT_QR_MODE_ALPHA_NUM    : return 13;
            case BLUE_PRODUCT_QR_MODE_8BIT_BYTE    : return 16;
            case BLUE_PRODUCT_QR_MODE_KANJI      : return 12;
            default :
                trigger_error("mode:$this->mode", E_USER_ERROR);
            }
...
class QRMath {
    ...
    static $BLUE_PRODUCT_QR_MATH_EXP_TABLE = null;
    static $BLUE_PRODUCT_QR_MATH_LOG_TABLE = null;

    static function init() {

        self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE = QRMath::createNumArray(256);

        for ($i = 0; $i < 8; $i++) {
            self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i] = 1 << $i;
        }

        for ($i = 8; $i < 256; $i++) {
            self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i] = self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i - 4]
                ^ self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i - 5]
                ^ self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i - 6]
                ^ self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i - 8];
        }

        self::$BLUE_PRODUCT_QR_MATH_LOG_TABLE = QRMath::createNumArray(256);

        for ($i = 0; $i < 255; $i++) {
            self::$BLUE_PRODUCT_QR_MATH_LOG_TABLE[self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$i] ] = $i;
        }
    }
    ...
    static function glog($n) {
        ...
        return self::$BLUE_PRODUCT_QR_MATH_LOG_TABLE[$n];
    }
    ...
    static function gexp($n) {
        ...
        return self::$BLUE_PRODUCT_QR_MATH_EXP_TABLE[$n];
    }
}
...
define("BLUE_PRODUCT_QR_MODE_NUMBER", 1 << 0);
define("BLUE_PRODUCT_QR_MODE_ALPHA_NUM", 1 << 1);
define("BLUE_PRODUCT_QR_MODE_8BIT_BYTE", 1 << 2);
define("BLUE_PRODUCT_QR_MODE_KANJI", 1 << 3);

//---------------------------------------------------------------
// MaskPattern
//---------------------------------------------------------------

define("BLUE_PRODUCT_QR_MASK_PATTERN000", 0);
define("BLUE_PRODUCT_QR_MASK_PATTERN001", 1);
define("BLUE_PRODUCT_QR_MASK_PATTERN010", 2);
define("BLUE_PRODUCT_QR_MASK_PATTERN011", 3);
define("BLUE_PRODUCT_QR_MASK_PATTERN100", 4);
define("BLUE_PRODUCT_QR_MASK_PATTERN101", 5);
define("BLUE_PRODUCT_QR_MASK_PATTERN110", 6);
define("BLUE_PRODUCT_QR_MASK_PATTERN111", 7);

//---------------------------------------------------------------
// ErrorCorrectLevel

// 7%.
define("BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_L", 1);
// 15%.
define("BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_M", 0);
// 25%.
define("BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_Q", 3);
// 30%.
define("BLUE_PRODUCT_QR_ERROR_CORRECT_LEVEL_H", 2);
